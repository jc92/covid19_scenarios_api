...
function advanceState(
  ...
  const update = (age: any, kind: any, compartment: any) => {
    ;(newPop as any)[kind][compartment][age] = gz(
      (pop as any)[kind][compartment][age] + dt * (tdot as any)[kind][compartment][age],
    )
  }

  const updateAt = (age: any, kind: any, compartment: any, i: any) => {
    ;(newPop as any)[kind][compartment][age][i] = gz(
      (pop as any)[kind][compartment][age][i] + dt * (tdot as any)[kind][compartment][age][i],
    )
  }
  ...
  trajectory.forEach((d) => {
    ...
    Object.keys(tp.current).forEach((k) => {
      if (k === 'exposed') {
        ...
        const tc = tp.current as any
        const dc = d.current as any
        ages.forEach((age, i) => {
          tc[k][age] = dc[k][i]
        })
        tc[k].total = dc[k].reduce((a: any, b: any) => a + b)
      }
    })

    Object.keys(tp.cumulative).forEach((k) => {
      const tc = tp.cumulative as any
      const dc = d.cumulative as any
      ages.forEach((age, i) => {
        tc[k][age] = dc[k][i]
      })
      tc[k].total = dc[k].reduce((a: any, b: any) => a + b, 0)
    })
