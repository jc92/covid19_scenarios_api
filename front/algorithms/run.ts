...
export function intervalsToTimeSeries(intervals: MitigationIntervals): TimeSeries {
  ...
  intervals.forEach((element) => {
    ...
    if (changePoints[(element.timeRange.tMin as any) as string] !== undefined) {
      changePoints[(element.timeRange.tMin as any) as string].push(val)
    } else {
      changePoints[(element.timeRange.tMin as any) as string] = [val]
    }
    // add inverse of the value when measure is relaxed
    if (changePoints[(element.timeRange.tMax as any) as string] !== undefined) {
      changePoints[(element.timeRange.tMax as any) as string].push(1.0 / val)
    } else {
      changePoints[(element.timeRange.tMax as any) as string] = [1.0 / val]
    }
  })
