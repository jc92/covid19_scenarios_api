...
import { SeverityTableRow } from '../types';
...
export function getPopulationParams(
  ...
  severity.forEach(d => {
    ...
    const isolated = d.isolated === undefined ? 0 : d.isolated;
...
export function evolve(pop: SimulationTimePoint, P: ModelParams, sample: (x: number) => number): SimulationTimePoint {
  ...
  // TODO: @bruno_rzn: here i've set specific keys (exposed or not) so the types match
  const update = (sub: keyof InternalCurrentData, age: string, delta: number) => {
    if (sub !== 'exposed') {
      newPop.current[sub][age] = pop.current[sub][age] + delta;
    } else {
      throw new Error('cannot use update function on "exposed" : use updateAt instead');
    }
  };
